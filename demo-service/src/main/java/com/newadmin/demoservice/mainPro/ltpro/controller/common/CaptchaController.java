package com.newadmin.demoservice.mainPro.ltpro.controller.common;
...
import com.newadmin.demoservice.config.core.utils.TemplateUtils;
import com.newadmin.demoservice.config.core.utils.ValidationUtils;
import com.newadmin.demoservice.config.messaging.mail.util.MailUtils;
import com.newadmin.demoservice.config.properties.CaptchaProperties;
import com.newadmin.demoservice.config.security.limiter.annotation.RateLimiter;
import com.newadmin.demoservice.config.security.limiter.annotation.RateLimiters;
import com.newadmin.demoservice.config.security.limiter.enums.LimitType;
import com.newadmin.demoservice.mainPro.ltpro.auth.model.resp.CaptchaResp;
...
public class CaptchaController {
    ...
    @GetMapping("/behavior")
    public JsonObject getBehaviorCaptcha(CaptchaVO captchaReq, HttpServletRequest request) {
        captchaReq.setBrowserInfo(
            JakartaServletUtil.getClientIP(request) + request.getHeader(HttpHeaders.USER_AGENT));
        ...
        return new JsonObject(responseModel.getRepData());
    }

    @Operation(summary = "校验行为验证码", description = "校验行为验证码")
    @PostMapping("/behavior")
    public JsonObject checkBehaviorCaptcha(@RequestBody CaptchaVO captchaReq,
        HttpServletRequest request) {
        captchaReq.setBrowserInfo(
            JakartaServletUtil.getClientIP(request) + request.getHeader(HttpHeaders.USER_AGENT));
        return new JsonObject(behaviorCaptchaService.check(captchaReq));
    }

    @Operation(summary = "获取图片验证码", description = "获取图片验证码（Base64编码，带图片格式：data:image/gif;base64）")
    @GetMapping("/image")
    public JsonObject getImageCaptcha() {
        ...
        return new JsonObject(
            CaptchaResp.builder().uuid(uuid).img(captcha.toBase64()).expireTime(expireTime)
                .build());
    }
     ...
     *
     * <p>
     * 限流规则：<br> 1.同一邮箱同一模板，1分钟2条，1小时8条，24小时20条 <br> 2、同一邮箱所有模板 24 小时 100 条 <br> 3、同一 IP 每分钟限制发送 30
     * 条
     * </p>
     *
     * @param email 邮箱
    ...
    @GetMapping("/mail")
    @RateLimiters({
        @RateLimiter(name = CacheConstants.CAPTCHA_KEY_PREFIX
            + "MIN", key = "#email + ':' + T(cn.hutool.extra.spring.SpringUtil).getProperty('captcha.mail.templatePath')", rate = 2, interval = 1, unit = RateIntervalUnit.MINUTES, message = "获取验证码操作太频繁，请稍后再试"),
        @RateLimiter(name = CacheConstants.CAPTCHA_KEY_PREFIX
            + "HOUR", key = "#email + ':' + T(cn.hutool.extra.spring.SpringUtil).getProperty('captcha.mail.templatePath')", rate = 8, interval = 1, unit = RateIntervalUnit.HOURS, message = "获取验证码操作太频繁，请稍后再试"),
        @RateLimiter(name = CacheConstants.CAPTCHA_KEY_PREFIX
            + "DAY'", key = "#email + ':' + T(cn.hutool.extra.spring.SpringUtil).getProperty('captcha.mail.templatePath')", rate = 20, interval = 24, unit = RateIntervalUnit.HOURS, message = "获取验证码操作太频繁，请稍后再试"),
    ...
    public JsonObject getMailCaptcha(
        ...
        MailUtils.sendHtml(email, "【%s】邮箱验证码".formatted(projectProperties.getName()),
            content);
        // 保存验证码
        String captchaKey = CacheConstants.CAPTCHA_KEY_PREFIX + email;
        RedisUtils.set(captchaKey, captcha, Duration.ofMinutes(expirationInMinutes));
        return new JsonObject("发送成功，验证码有效期 %s 分钟".formatted(expirationInMinutes));
    }
     ...
     *
     * <p>
     * 限流规则：<br> 1.同一号码同一模板，1分钟2条，1小时8条，24小时20条 <br> 2、同一号码所有模板 24 小时 100 条 <br> 3、同一 IP 每分钟限制发送 30
     * 条
     * </p>
     *
     * @param phone      手机号
    ...
    @RateLimiters({
        @RateLimiter(name = CacheConstants.CAPTCHA_KEY_PREFIX
            + "MIN", key = "#phone + ':' + T(cn.hutool.extra.spring.SpringUtil).getProperty('captcha.sms.templateId')", rate = 2, interval = 1, unit = RateIntervalUnit.MINUTES, message = "获取验证码操作太频繁，请稍后再试"),
        @RateLimiter(name = CacheConstants.CAPTCHA_KEY_PREFIX
            + "HOUR", key = "#phone + ':' + T(cn.hutool.extra.spring.SpringUtil).getProperty('captcha.sms.templateId')", rate = 8, interval = 1, unit = RateIntervalUnit.HOURS, message = "获取验证码操作太频繁，请稍后再试"),
        @RateLimiter(name = CacheConstants.CAPTCHA_KEY_PREFIX
            + "DAY'", key = "#phone + ':' + T(cn.hutool.extra.spring.SpringUtil).getProperty('captcha.sms.templateId')", rate = 20, interval = 24, unit = RateIntervalUnit.HOURS, message = "获取验证码操作太频繁，请稍后再试"),
    ...
    public JsonObject getSmsCaptcha(
        @NotBlank(message = "手机号不能为空") @Pattern(regexp = RegexPool.MOBILE, message = "手机号格式错误") String phone,
