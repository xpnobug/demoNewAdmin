...
public class BehaviorCaptchaCacheAutoConfiguration {
    ...
    public static class Default {
        ...
        public void postConstruct() {
            CaptchaServiceFactory.cacheService.put(StorageType.DEFAULT.name().toLowerCase(),
                captchaCacheService());
            log.debug(
                "[New Admin] - 自动配置'Captcha-Behavior-Cache-Default'完成初始化。");
        }
    }
    ...
    public static class Redis {
        ...
        public void postConstruct() {
            CaptchaServiceFactory.cacheService.put(StorageType.REDIS.name().toLowerCase(),
                captchaCacheService());
            log.debug(
                "[New Admin] - 自动配置'Captcha-Behavior-Cache-Redis'完成初始化。");
        }
    }
    ...
    public static class Custom {
        ...
        public void postConstruct() {
            ...
            log.debug(
                "[New Admin] - 自动配置'Captcha-Behavior-Cache-Custom'完成初始化.");
        }
    }
}
