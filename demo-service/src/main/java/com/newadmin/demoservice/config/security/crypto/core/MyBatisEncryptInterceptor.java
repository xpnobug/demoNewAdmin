 ...
 * @since 1.4.0
 */
@Intercepts({@Signature(method = "update", type = Executor.class, args = {MappedStatement.class,
    Object.class}),
...
public class MyBatisEncryptInterceptor extends AbstractMyBatisInterceptor {
     ...
     * @throws Exception /
     */
    private void encryptMap(HashMap<String, Object> parameterMap, MappedStatement mappedStatement)
        throws Exception {
        ...
        for (Map.Entry<String, FieldEncrypt> encryptParamEntry : encryptParamMap.entrySet()) {
            ...
            if (parameterName.startsWith(Constants.ENTITY)) {
                ...
                parameterMap.put(parameterName,
                    this.doEncrypt(parameterMap.get(parameterName), fieldEncrypt));
            }
        }
    }
    ...
    private void doEncrypt(List<Field> fieldList, Object entity) throws Exception {
        for (Field field : fieldList) {
            ...
            String password = ObjectUtil.defaultIfBlank(
                field.getAnnotation(FieldEncrypt.class).password(), properties
                    .getPassword());
            String ciphertext = encryptor.encrypt(fieldValue.toString(), password,
    ...
    private void doEncrypt(Wrapper wrapper, MappedStatement mappedStatement) throws Exception {
        if (wrapper instanceof AbstractWrapper abstractWrapper) {
            ...
            String className = CharSequenceUtil.subBefore(mappedStatement.getId(),
                StringConstants.DOT, true);
            Class<?> mapperClass = Class.forName(className);
            Optional<Class> baseMapperGenerics = getEntityTypeByMapperClass(mapperClass,
                Optional.empty());
            // 获取不到泛型对象 则不进行下面的逻辑
            ...
            for (String sqlFragment : sqlSet.split(Constants.COMMA)) {
                ...
                String paramNameVal = sqlFragment.split(Constants.EQUALS)[1].substring(25,
                    sqlFragment
                ...
                if (fieldInfo.isPresent()) {
                    ...
                    FieldEncrypt fieldEncrypt = tableFieldInfo.getField()
    ...
    private Object doEncrypt(Object parameterValue, FieldEncrypt fieldEncrypt) throws Exception {
        ...
        // 优先获取自定义对称加密算法密钥，获取不到时再获取全局配置
        String password = ObjectUtil.defaultIfBlank(fieldEncrypt.password(),
     ...
     * @return 泛型
     */
    private static Optional<Class> getEntityTypeByMapperClass(Class<?> mapperClass,
        Optional<Class> tempResult) {
        ...
        for (Type genericInterface : genericInterfaces) {
            if (genericInterface instanceof ParameterizedType parameterizedType) {
                ...
                if (rawType.equals(BaseMapper.class)) {
                    ...
                    // 递归调用，继续查找
                    Optional<Class> innerResult = getEntityTypeByMapperClass(interfaceClass,
                        result);
