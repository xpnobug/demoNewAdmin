...
//        int cursor = 0;
//        for (int start, end; (start = message.indexOf(StringConstants.DOLLAR_LEFT_BRACE, cursor)) != -1
...
//            sb.append(value == null ? StringConstants.EMPTY : value);
//            cursor = end + 1;
//        }
...
//    }
    ...
    public static String getId(Random random, int len, int radix) {
        if (len < 8) {
            throw new IllegalArgumentException(
                "为了减少冲突，len 需要大于7，实际尽量设置在10~16或以上。");
        }
    ...
    public static String cleanText(@Nullable String txt) {
        ...
        return SPECIAL_CHARS_REGEX.matcher(txt).replaceAll(StringConstants.EMPTY);
    }
     ...
     * of the String will be returned without an exception. If the String is {@code null},
     * {@code null} will be returned. An empty String is returned if len is negative or exceeds the
